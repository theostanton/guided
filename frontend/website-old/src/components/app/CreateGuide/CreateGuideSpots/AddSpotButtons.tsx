...
export default class AddSpotButtons extends React.Component<Props, State> {
  ...
  render(): React.ReactElement {
    const { position, addAtIndex, createGuideStore } = this.props

    const isCircular =
      createGuideStore!.guide && createGuideStore!.guide.isCircular

    if (this.props.position !== "firstLast") {
      return (
        <div
          key={`add_${addAtIndex ? addAtIndex : "end"}`}
          style={{
            width: "min-content",
            marginLeft: "auto",
            marginRight: "auto",
        ...
        >
          <Button
            ...
            onClick={async () => {
              ...
              await createGuideStore.addSpot(addAtIndex)
              this.setState({
                addingSpot: false,
              })
            }}
