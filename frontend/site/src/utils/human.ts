...
export type Duration = {
  ...
  unitShort: 'mins' | 'min' | 'h'
}

export function duration(durationSeconds: number): Duration {
  const durationMinutes = durationSeconds / 60
  switch (true) {
    ...
    default:
      ...
      return {
        unitLong: durationHours === 1 ? 'hour' : 'hours',
        unitShort: 'h',
        value: Math.ceil(durationMinutes / 60)
      }
  }
}
...
export function humanCountry(countryCode: string): string {
