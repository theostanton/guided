...
export default async function (_: void, {guideId, locked, label, address: addressInput}: { guideId: number, locked: boolean, label: string, address: Partial<Address> }): Promise<number | null> {
    ...
    const spot = await daos.spot.insert({
        location: loctionId
    });
