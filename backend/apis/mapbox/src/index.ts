...
export async function uploadFeature(key: string, feature: Feature): Promise<void> {

  const datasetResponse = await datasetClient.createDataset({
    name: key,
  }).send()
  ...
  const featureResponse = await datasetClient.putFeature({
    datasetId: datasetId,
    featureId: key,
    feature,
  }).send()
