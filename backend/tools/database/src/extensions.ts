...
export type Table = "stages" | "rides" | "spots" | "computations" | "users" | "guides"
...
export function extend(db: IDatabase<Extensions> & Extensions) {
  const instance: Extensions = {
    ...
    selectGuide(guideId: string): Promise<Guide> {
      return db.one<Guide>(`SELECT *
                            from guides
                            where id = $1`, [guideId])
    },
    selectSpot(spotId: string): Promise<Spot> {
      return db.one<Spot>(`SELECT *
                           from spots
                           where id = $1`, [spotId])
    },
    selectRide(rideId: string): Promise<Ride> {
      return db.one<Ride>(`SELECT *
                           from rides
                           where id = $1`, [rideId])
    },
    selectSpotsForGuide(guideId: string): Promise<Spot[]> {
    ...
    async updateMany<T>(table: Table, items: Patch<T>[]): Promise<ItemId[]> {
      if (items.length === 0) {
        return []
      }
      ...
      const query = updateMany(table, items, Object.keys(items[0]), "id", undefined)
