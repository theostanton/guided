...
export function updateMany(
    ...
    columns: string[],
    onColumn = 'id',
    oldOnColumnValue: string | undefined = undefined
): string {
    ...
    const condition = oldOnColumnValue
        ? `t."${onColumn}" = '${oldOnColumnValue}'`
        : `t."${onColumn}" = c."${onColumn}"`

    return `
    update ${tableName} as t
    set ${sets}
    from (values${values}) as c("${onColumn}"${
...
export function updateOne<T>(
    ...
    const condition = `${onColumn} = '${
        oldOnColumnValue || (item as any)[onColumn]
    }'`
...
export function insertMany(
    tableName: string,
    items: any[],
    returning: string | undefined = undefined
): string {
