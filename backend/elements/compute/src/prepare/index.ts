...
export default async function(guideId: string): Promise<Packet> {
  ...
  if (lockedSpots.length > 1) {
    ...
    const { isCircular } = await database.one<{ isCircular: boolean }>(`select is_circular as "isCircular"
                                                                        from guides
                                                                        where id = $1`, [guideId])

    const totalLockedSpots = isCircular ? lockedSpots.length : lockedSpots.length - 1
    for (let i = 0; i < totalLockedSpots; i++) {
      ...
      const stage = stages.find(stage => {
        return stage.from_spot === thisSpotId && stage.to_spot === nextSpotId && stage.status !== "stale"
      })
      if (stage) {
  ...
  const insertStagesResult = await database.insertMany("stages", stagesToCreate)

  if (!insertStagesResult || insertStagesResult.length != stagesToCreate.length) {
  ...
  const stageIdsToUpdate = stages.filter(stage => {
  ...
  await Promise.all(packet.alteredStages.map(stage => {
  ...
  const createComputationsResult = await database.insertMany("computations", computations)

  if (!createComputationsResult || createComputationsResult.length !== computations.length) {
