...
async function runFinalisationIfRequired(guide: Guide): Promise<boolean> {
  const activeComputations = await database.manyOrNone(
    "select id from computations where guide=$1 and status in ('computing','scheduled')",
    [guide.id],
  )
...
async function insert(data: StageData): Promise<void> {
...
export default async function(
  ...
  try {
    await database.updateOne<Computation>("computations", {
      id: computationId,
      status: "computing",
    })
    ...
    const fromSpot = await database.one<Spot>(
      "select * from spots where id=$1",
      [stage.from_spot],
    )
    const toSpot = await database.one<Spot>(
      "select * from spots where id=$1",
      [stage.to_spot],
    )
    ...
    const mode: "driving" | "bicycling" | "walking" =
      guide.transport_type === "BICYCLE" ? "bicycling" : "driving"
    ...
    const stageData = await calculateStage(
      startDate,
      stage.id,
      guide,
      fromSpot,
    ...
    const ranFinalisation = await runFinalisationIfRequired(guide)
