...
import { CreateGuideResult, TransportType, UpdateGuideResult } from "../../generated"
...
describe("When updating a guides title", () => {
  ...
  it("Should have created original", async () => {
    ...
    const resultForQueryingOldId = await database.oneOrNone<Guide>("select * from guides where id=$1", [createGuideResult.guideId])
  ...
  it("Should have updated the guide", async () => {
    ...
    expect(updatedGuide.updated.getTime()).toBeLessThan(new Date().getTime())
  })
