...
import { LOCATIONS, MockLocation } from "@guided/spinup/srv/builder/GuideBuilder"
...
async function spinupSpots(isCircular: boolean = false, startDate: string | null = START_DATE, spotCount: number = 2): Promise<Initial> {
  ...
  const content = UserBuilder.create()
    .addGuide(faker.random.words(1), (builder => {
      ...
      builder.nextSpotLocation(spot1.location, spot1.nights, spot1.id, spot1.label)
      builder.nextSpotLocation(spot2.location, spot2.nights, spot2.id, spot2.label)
      if (spotCount > 2) {
        spot3 = {
          id: generateId("spot"),
          label: "Spot 3",
        ...
        builder.nextSpotLocation(spot3.location, spot3.nights, spot3.id, spot3.label)
      }
    }))
    .build()
  ...
  let guideId: string = content.guides[0].id
...
describe("When setting nights to 3 on a circular guide", () => {
  ...
  it("Should ammend dates for each ride", async () => {
    ...
    const rides = await database.manyOrNone(`select id
                                             from rides
                                             where guide = $1`, [initial.guideId])
    expect(rides.length).toBe(2)

    const ride1 = await database.one<Ride>(`select *
                                            from rides
                                            where from_spot = $1`, [initial.spot1.id])
    expect(ride1.date).toBe("2019-06-01")

    const ride2 = await database.one<Ride>(`select *
                                            from rides
                                            where from_spot = $1`, [initial.spot2.id])
...
describe("When editing a spots location", () => {
  ...
  it("Should stagnate 2 stages", async () => {
    const staleStages = await database.manyOrNone<Stage>(`select *
                                                          from stages
                                                          where guide = $1
                                                            and status = 'stale'`, [initial.guideId])
    expect(staleStages.length).toBe(2)
  })

  it("Should have computed 2 new stages", async () => {
    const newStages = await database.manyOrNone<Stage>(`select *
                                                        from stages
                                                        where guide = $1
                                                          and status = 'ready'
                                                          and created > $2`, [initial.guideId, timeBefore])
    expect(newStages.length).toBe(2)
  })

  it("Should have computed 5 stages total", async () => {
    const stages = await database.manyOrNone<Stage>(`select *
                                                     from stages
                                                     where guide = $1`, [initial.guideId])
