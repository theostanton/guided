...
describe('When moving a spot on a guide with 1 spot', () => {
    ...
    beforeAll(async () => {
        const contents = UserBuilder.create(faker.internet.email(), owner)
            .addGuide(GUIDE_TITLE, (builder) => {
                ...
                builder.nextSpot(
                    LOCATIONS.London.lat,
                    LOCATIONS.London.long,
                    1,
                    undefined,
                    SPOT_ID
                )
    ...
    it('Updated a spots ID', async () => {
        ...
        const forOldSpotId = await database.manyOrNone<Spot>(
            'select * from spots where id=$1',
            [SPOT_ID]
        )
        expect(forOldSpotId.length).toBe(0)

        const forNewSpotId = await database.manyOrNone<Spot>(
            'select * from spots where id=$1',
            [newId]
        )
    ...
    it('Updated a spot', async () => {
        const spot = await database.one<Spot>(
            'select * from spots where id=$1',
            [newId]
        )
...
describe('When moving a spot on a guide with 2 spots', () => {
    ...
    beforeAll(async () => {
        const contents = UserBuilder.create(faker.internet.email(), owner)
            .addGuide(GUIDE_TITLE, (builder) => {
                ...
                builder.nextSpot(
                    LOCATIONS.Brighton.lat,
                    LOCATIONS.Brighton.long,
                    1,
                    undefined,
                    SPOT_ID_2
                )
    ...
    it("Create 2 stages in 'computing' stage", async () => {
        const stages = await database.manyOrNone<Stage>(
            'select stages.* from stages inner join spots on stages.from_spot=spots.id where stages.guide=$1 order by spots.position',
            guideId
        )
        expect(stages.length).toBe(2)
    ...
    it('Schedule 2 computations to be executed ', async () => {
        expect(packet.computationIds.length).toBe(2)
        const computations = await database.manyOrNone<Computation>(
            'select * from computations where guide=$1 and created>$2',
            [guideId, dateBefore]
        )
    ...
    describe('After first execution', () => {
        ...
        it("Should have updated 2 stages as 'ready'", async () => {
            const stages = await database.manyOrNone<Stage>(
                'select * from stages where guide=$1',
                [guideId]
            )
            expect(stages.length).toBe(2)
...
describe('When moving a spot on a guide with 3 spots', () => {
    ...
    beforeAll(async () => {
        const contents = UserBuilder.create(faker.internet.email(), owner)
            .addGuide(GUIDE_TITLE, (builder) => {
                ...
                builder.nextSpot(
                    LOCATIONS.Worthing.lat,
                    LOCATIONS.Worthing.long,
                    1,
                    undefined,
                    SPOT_ID_3
                )
    ...
    describe('After first execution', () => {
        ...
        it('marked all computations as success', async () => {
            const computations = await database.manyOrNone<Computation>(
                "select * from computations where guide=$1 and status!='success'",
                [guideId]
            )
